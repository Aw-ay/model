...
def _emit_verification(request: ConnectedShellRequest, rfdc: str, properties: tuple[tuple[str, str], ...], ps_properties: tuple[tuple[str, str], ...], external_rf: tuple[RfdcProbeInterface, ...], mts_properties: tuple[tuple[str, str], ...], realization_sha: str) -> bytes:
    ...
    lines += [
        ...
        "  set reset_from_pin {}",
        "  foreach candidate $vendor_reset_from_candidates {",
        "    set candidate_name [get_property NAME $candidate]",
        "    if {[regexp {rx_reset_0/U0/ACTIVE_LOW_PR_OUT_DFF\\[0\\]\\.FDRE_PER_N/C$} $candidate_name]} { lappend reset_from_pin $candidate }",
        "  }",
        "  set reset_to_pin {}",
        "  foreach candidate $vendor_clk_valid_to_candidates {",
        "    set candidate_name [get_property NAME $candidate]",
        "    if {[regexp [format {cdc_adc%d_clk_valid_i/syncstages_ff_reg\\[0\\]/D$} $tile] $candidate_name]} { lappend reset_to_pin $candidate }",
        "  }",
        "  if {[llength $reset_from_pin] != 1 || [llength $reset_to_pin] != 1} { error {AMD RFDC CDC-11 ADC waiver endpoint discovery mismatch} }",
        "  create_waiver -user $vendor_waiver_user -type CDC -id CDC-11 -tags $vendor_waiver_tag -description {Passing fabric reset into the RFDC clock-valid synchronizer} -from $reset_from_pin -to $reset_to_pin",
        "}",
        "foreach tile {0 1} {",
        "  set vendor_reset_from_candidates [get_pins -hier -quiet -filter {NAME =~ */tx_reset_0/U0/ACTIVE_LOW_PR_OUT_DFF* && REF_PIN_NAME == C}]",
        "  set vendor_clk_valid_to_candidates [get_pins -hier -quiet -filter [format {NAME =~ */rfdc_0/inst/cdc_dac%d_clk_valid_i/syncstages_ff_reg*/D} $tile]]",
        "  set reset_from_pin {}",
        "  foreach candidate $vendor_reset_from_candidates {",
        "    set candidate_name [get_property NAME $candidate]",
        "    if {[regexp {tx_reset_0/U0/ACTIVE_LOW_PR_OUT_DFF\\[0\\]\\.FDRE_PER_N/C$} $candidate_name]} { lappend reset_from_pin $candidate }",
        "  }",
        "  set reset_to_pin {}",
        "  foreach candidate $vendor_clk_valid_to_candidates {",
        "    set candidate_name [get_property NAME $candidate]",
        "    if {[regexp [format {cdc_dac%d_clk_valid_i/syncstages_ff_reg\\[0\\]/D$} $tile] $candidate_name]} { lappend reset_to_pin $candidate }",
        "  }",
        "  if {[llength $reset_from_pin] != 1 || [llength $reset_to_pin] != 1} { error {AMD RFDC CDC-11 DAC waiver endpoint discovery mismatch} }",
        "  create_waiver -user $vendor_waiver_user -type CDC -id CDC-11 -tags $vendor_waiver_tag -description {Passing fabric reset into the RFDC clock-valid synchronizer} -from $reset_from_pin -to $reset_to_pin",
        "}",
        "foreach tile {0 1 2 3} {",
