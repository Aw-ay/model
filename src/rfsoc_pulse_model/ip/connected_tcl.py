...
def _emit_verification(request: ConnectedShellRequest, rfdc: str, properties: tuple[tuple[str, str], ...], ps_properties: tuple[tuple[str, str], ...], external_rf: tuple[RfdcProbeInterface, ...], mts_properties: tuple[tuple[str, str], ...], realization_sha: str) -> bytes:
    lines = [
        ...
        "set validate_result [validate_bd_design -quiet]", "if {[llength $validate_result] != 0} { error {validate_bd_design returned violations} }", "set connected_bd_file [get_files -quiet *.bd]", "if {[llength $connected_bd_file] != 1} { error {connected BD file is not unique} }", "generate_target all $connected_bd_file", "set wrapper [make_wrapper -files $connected_bd_file -top]", "add_files -norecurse $wrapper", "set_property top connected_rfdc_shell_wrapper [current_fileset]", "launch_runs synth_1 -jobs 1", "wait_on_run synth_1", "set synth_status [get_property STATUS [get_runs synth_1]]", "if {$synth_status ne {synth_design Complete!}} { error {synthesis incomplete} }", "open_run synth_1",
