...
class RxGroupIngress2Spc(RTLModule):
    ...
    There is deliberately no ready input. A complete 16-stream valid group is
    accepted atomically; a partial group is dropped and sets a sticky error.
    """
    ...
    def compute(self) -> None:
        ...
        accepted = all_valid & (~self.format_error_o)
        all_idle = self.adc_i_tvalid_i.eq(0) & self.adc_q_tvalid_i.eq(0)
        partial_group = (~all_idle) & (~all_valid)
