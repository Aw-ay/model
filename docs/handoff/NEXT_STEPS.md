...
- **Current candidate evidence:** The existing `rx_2spc_continuous_ingress`, `continuous_stream_timebase`, and `tx_2spc_continuous_egress` Cycle candidates retain their 13 focused tests and deterministic Verilog evidence. The new `rx_2spc_calibrated_hv_frontend` candidate passes 13 focused tests, including normalized Golden identity-profile quantization across all nine H/V range pairs, negative and authority-rail vectors, consecutive two-sample beats, and byte-identical Verilog with complete port widths. All four remain outside the production registry and do not alter authority configuration.
...
- **Required tests/evidence:** Cycle implementations for 2SPC ingress, calibrated H/V selection, delay, polarimetric scattering, Doppler, accumulation, predistortion and egress; generated RTL only; complete bit/cycle equivalence against the production Golden contract; continuous throughput and fault tests; synthesis resource/timing evidence. The current calibrated H/V evidence is a candidate-boundary result only and does not close receive matrix inversion, delay, reflection, monitor, DMA, timing, runtime MTS, or board acceptance.
